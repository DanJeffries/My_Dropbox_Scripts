...
### Using two illumina shotgun read files (Spain_0_ and Spain _1_)

setwd("~/Data/Transposable_elements/dnaPipeTE/shotSpain_0_1_concat_outputs/")

#read the table with reads and identity selected for landscape analysis
file1 = "~/Data/Transposable_elements/dnaPipeTE/shotSpain_0_1_concat_outputs/reads_landscape"
file2 = "~/Data/Transposable_elements/dnaPipeTE/shotSpain_0_1_concat_outputs/factors_and_colors"
...
#plot the landscape graph
ggplot(land, aes(div, fill=fam1))+geom_histogram(binwidth=1.1)+labs(list(x="div", y="reads count"))+scale_x_continuous(limits = c(0, 35))+scale_fill_manual(values=as.character(fac_col$V3))+guides(fill=guide_legend(ncol=3))+theme(legend.direction ="vertical",legend.position = "right")
ggsave("landscape.pdf", height=12, width=15)
