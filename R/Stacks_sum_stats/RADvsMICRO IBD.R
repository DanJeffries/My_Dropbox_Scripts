...
M1_df <- read.fstat("~/Dropbox/PhD/Dans_PhD_Shared/Data/Microsatellites/DAPC//Complete dataset outputs/PurecruplusManc_stripped_and_checked_numbered.DAT")
pops <- read.delim("~/Dropbox/PhD/Dans_PhD_Shared/Data/Microsatellites/DAPC//Complete dataset outputs/PurecruplusManc_stripped_and_checked_indpops.txt", header = F)
pops
M1 <- df2genind(M1_df)
pop(M1) <- pops$V1
M1$pop <- pops$V1
MicroCoords <- read.csv("~/Dropbox/PhD/Dans_PhD_Shared/Data/Microsatellites/R stats/Adegenet IBD/Mantelcoordinates.csv",header = TRUE, row.names = 1) ##table containing xy coordinates
...
smoothScatter(M1H1Dgeo, M1H1Dgen, main = "a) M1 Hap1, no STEC data IBD", xlab = "Dgeo", ylab = "Ggen")
...
M2_df <- read.fstat("/home/dan/Dropbox/PhD/Dans_PhD_Shared//Data/RAD/RADsample_microsat_genotypes_numbered.dat") ## Load file - converts to GENIND object.
...
M2 <- df2genind(M2_df)
M2pops <- read.delim("/home/dan/Dropbox/PhD/Dans_PhD_Shared//Data/RAD/RADsample_microsat_POPS.txt", header = F)
...
smoothScatter(M1H1Dgeo, M1H1Dgen, nbin = 250,  xlab = "Dgeo", ylab = "Ggen")
points(M1H1Dgeo, M1H1Dgen, pch = ".", cex = 3)
title(main = "a) M1 Lineage 1 only (excluding NOR2)", adj = 0, cex.main = 0.8)
abline(lm(M1H1Dgen~M1H1Dgeo),  col="darkred",lty=2, lwd = 1.5)  ## Adds regression line
M1H1lm <- summary(lm(M1H1Dgen~M1H1Dgeo))
textxy(1,0.2, paste0("adj. R squared = ",round(M1H1lm$adj.r.squared,3),"***"), cex = 1)
...
smoothScatter(RADDgeo, RADDgen, nbin = 250, xlab = "Dgeo", ylab = "Ggen")
title(main = "b) RADseq data IBD", adj = 0, cex.main = 0.8)
...
smoothScatter(M2Dgeo, M2Dgen, nbin = 250,  xlab = "Dgeo", ylab = "Ggen")
points(M2Dgeo, M2Dgen, pch = ".", cex = 3)
title(main = "c) M2 data IBD", adj = 0, cex.main = 0.8)
abline(lm(M2Dgen~M2Dgeo),  col="darkred",lty=2, lwd = 1.5)  ## Adds regression line
M2lm <- summary(lm(M2Dgen~M2Dgeo))
textxy(1,0.3, paste0("adj. R squared = ",round(M2lm$adj.r.squared,3),"***"), cex = 1)


######## M3 ###########

smoothScatter(M3Dgeo, M3Dgen, nbin = 250, xlab = "Dgeo", ylab = "Ggen")
points(M3Dgeo, M3Dgen, pch = ".", cex = 3)
title( main = "d) M3 data IBD", adj= 0, cex.main = 0.8)
abline(lm(M3Dgen~M3Dgeo),  col="darkred",lty=2, lwd = 1.5)  ## Adds regression line
M3lm <- summary(lm(M3Dgen~M3Dgeo))
textxy(1,0.3, paste0("adj. R squared = ",round(M3lm$adj.r.squared,4),"***"), cex = 1)
M3lm

#title(main = "Isolation by distance signal compared between ~13000 SNPs and 13 microsatellites in 149 \n pure crucian carp in Europe", outer = T)
