...
body <- read.csv("C:/Users/Dan/Dropbox/PhD/Dan's PhD (Shared)/Data/Microsatellites/DAPC/Complete dataset outputs/ClustermembershipsbodnoSTEC.csv")  ## Load my cluster membership data file.
...
Popmean4clustmembsbodnoSTEC <- sapply(split(body[2:5],body$population),colMeans) ##Very handy script, "colMeans" calculates the mean of columns in the subgroups produced as a result of performing split(body[2:13],body$population).  I then wrote this to the .csv below.

Popmean4clustmembsbodnoSTEC  ## check it - note that it doesn't have population names

write.csv(Popmean4clustmembsbodnoSTEC, "C:/Users/Dan/Dropbox/PhD/Dan's PhD (Shared)/Data/Microsatellites/DAPC/Complete dataset outputs/Popmean4clustmembsbodnoSTEC.csv")   ## I manually added my population names and cloumn headers to the csv 

##Just add the below script after plotting the map.

pies <- read.csv("C:/Users/Dan/Dropbox/PhD/Dan's PhD (Shared)/Data/Microsatellites/DAPC/Complete dataset outputs/Popmean4clustmembsbodnoSTEC.csv", header=T) ## have read this in again and specified that there are headers as I was having difficulty assigning headers to the object. 

pies <- Popmean4clustmembsbodnoSTEC ## get rid of STEC population
pies
...
points(cords$lon, cords$lat, pch=19, col="red", cex=0.5)  ## Plots my sample locations on the map (just for fun). 
...
## Now plot the pies, note that these pies have been offset from their actual population locations so as to allow for minimal overlap ##
...
add.pie(pies  $  PED	,	x	=	12.34	,	y	=	55.73	,	labels	=	""	,	radius	=	0.9	,	edges	=	200,	clockwise	=	T, col = mycol)													add.pie(pies  $  COP	,	x	=	12.5	,	y	=	55.8	,	labels	=	""	,	radius	=	1.05	,	edges	=	200,	clockwise	=	T, col = mycol)																																			
...
add.pie(pies  $	OBY	,	x	=	17.79	,	y	=	60.21	,	labels	=	""	,	radius	=	1.448208334	,	edges	=	200,	clockwise	=	T, col = mycol)											
add.pie(pies  $	TROM	,	x	=	18.95	,	y	=	69.65	,	labels	=	""	,	radius	=	1.448208334	,	edges	=	200,	clockwise	=	T, col = mycol)										
add.pie(pies  $	WEN	,	x	=	18.95	,	y	=	59.66 ,	labels	=	""	,	radius	=	1.448208334	,	edges	=	200,	clockwise	=	T, col = mycol)																																			
