...
## Function to make a heatmap from a VCF

def Heatmap_from_VCF(vcf_path, popmap = None):
    ...
    for i in open(popmap, 'r').readlines():
        popdict[i.split()[0]] = i.split()[1]

    ## read data in from VCF
    variants = allel.read_vcf(vcf_path)
    var_gen_array = allel.GenotypeArray(variants["calldata/GT"])
    var_n_alts = var_gen_array.to_n_alt(fill = -1) ## convert to number of ALT alleles

    ## get real sample names
    if popmap:  ## Attach sex to the samples if popmap is provided
        samples = []
        for sample in variants["samples"]:
            if sample in popdict:
                samples.append("%s_%s" % (sample, popdict[sample]))
            else:
                samples.append("%s_%s" % (sample, "unkn"))
    else:
        samples = variants["samples"]

    ## make the clusterplot
    cm = seaborn.clustermap(var_n_alts, figsize=(20,20))

    ## remake sample name list based on the clustering dendograms
    clustered_sample_index = cm.dendrogram_col.dendrogram["leaves"]
    clustered_sample_labels = []
    for i in clustered_sample_index:
        clustered_sample_labels.append(samples[i])

    ## Do the same for the tag IDs

    loci = variants["variants/ID"]

    clustered_locus_index = cm.dendrogram_row.dendrogram["leaves"]

    clustered_locus_labels = []
    for i in clustered_locus_index:
        clustered_locus_labels.append(loci[i])

    ## Add the axis labels
    plt.setp(cm.ax_heatmap.set_yticklabels(labels = clustered_locus_labels), rotation = 0)
    plt.setp(cm.ax_heatmap.set_xticklabels(labels = clustered_sample_labels), rotation = 90)


    # label the colorbar
    cm.ax_heatmap.collections[0].colorbar.set_label("Genotype")
    cm.ax_heatmap.collections[0].colorbar.set_ticks([-1,0,1,2])
    cm.ax_heatmap.collections[0].colorbar.set_ticklabels(["NA","0/0","0/1","1/1"],update_ticks=True)

    ## Save the plot
    outpdf = "%s_clusterplot.pdf" % vcf_path.rpartition(".")[0]
    plt.savefig(outpdf)
    print("Clusterplot PDF saved here: %s" % outpdf)

    ## Show the plot
    plt.show()


## Function to make a PCA from a VCF

def PCA_from_VCF(vcf_path):
    
    from matplotlib import pyplot as plt
    import allel
    
    ## import vcf as numpy arrary

    variants = allel.read_vcf(vcf_path)
    var_gen_array = allel.GenotypeArray(variants["calldata/GT"])
    var_n_alts = var_gen_array.to_n_alt() ## convert to number of ALT alleles

    samples = variants["samples"] ## sample labels used in PCA later

    ## Run PCA

    coords1, model1 = allel.pca(var_n_alts, n_components=10, scaler='patterson')

    ## Plot (using sample names from allel.genotypes array)
    ...
    x = [i[PCX] for i in coords1]
    y = [i[PCY] for i in coords1]

    plt.subplot(1,2,1)
    plt.scatter(x, y)

    for i in range(len(x)):
        plt.text(x[i]+.1, y[i], samples[i], fontsize = 8)

    plt.title("PC_%s Vs. PC_%s" % (PCX, PCY))
    plt.xlabel("PC_%s" % PCX)
    plt.ylabel("PC_%s" % PCY)
...
## Function to filter a VCF for specific loci and output a new VCF
...
def missing_data_finder(input_file, output_file_path, blacklisted_tags_cutoff=100):
    ...
    sorted_tags_per_sample = sorted(list(missing_data_count.items()), key=itemgetter(1), reverse=True) ## write file from this sorted tags list
    ...
    plt.yticks(list(range(0,max(y),100)), fontsize = 30)
    ...
    sorted_tags = sorted(list(missing_data_tags.items()), key=itemgetter(1), reverse=True)
    ...
    plt.yticks(list(range(0,max(y),50)), fontsize = 30)
    plt.xticks(list(range(0,max(x),100)), fontsize = 30, rotation = "vertical")
    plt.ylabel("Number of dropouts", fontsize = 30)
...
def missing_data_per_pop(missing_data_per_sample_dict, pop_file_path, outpath):
    ...
    x = list(pop_missing_dict.keys())
    y = [sum (i) for i in list(pop_missing_dict.values())]
    z = list(zip(x,y))            
    ## write to file
    ...
    plt.figure(figsize=(70,50))
    plt.bar(list(range(len(x))), y, align='center', width = 0.6)
    plt.xticks(list(range(len(x))), x, fontsize = 30)
    plt.yticks(list(range(0,max(y),(np.round(max(y)/10)))),list(range(0,max(y),(np.round(max(y)/10)))) , fontsize = 30)
...
def Super_av_tag_cov(vcf_path, whitelist = None, popmap = None):
    ...
    if not popmap == None:
        ...
        ax.set_xticks(list(range(1, len(pop_sample_dict_means)+1)))
...
def sample_Avcov_from_vcf(vcf_path):
    ...
    for record in myvcf:
        ...
        for sample in record.samples:
            if not sample['GT'] == "./.":  ## disregard missing loci!!
                if sample.sample not in list(sample_cov.keys()):
    ...
    y = []
    sample_count = 0
    for sample in list(sample_cov.keys()):
        sample_count += 1
    ...
    print("Mean sample coverage = %s (+/%s) " % (np.round(np.mean(list(sample_avgs.values())),2), np.round(np.std(list(sample_avgs.values())),2)))
...
def SL_snp_freq(myvcfpath, popmappath, catalog_tags_file, X_or_Z_freq_threshold = 0.4, sample_presence_cutoff = 0.75, coverage_threshold = 3, maf_threshold = 0.05, homgam_REF_freq = 0.95):    
    ...
    for record in vcf_reader:
        ...
        elif record.aaf[0] >= maf_threshold:
...
def SL_tag_finder(catalog_tags_file, popmappath, sex_presence_thresh = 0.5):
    ...
    for tag in cat_file[1:]:
        ...
        if binary == True:
            tag_fields = samples_field.split(b",")
        elif binary == False:
            tag_fields = samples_field.split(",")

        for sample in tag_fields:
            if binary == True:
                sample_ID = sample.split(b"_")[0]
            elif binary == False:
...
def SL_markers_from_sexed_fam(vcf_path, sex_info_file, catalog_path, offspring_presence_threshold, mendelian_threshold, het_sex_thresh, hom_sex_thresh):
    ...
    for record in myvcf:
        ...
        elif len(Par_allele_counts) == 2 and not 2 in list(Par_allele_counts.values()): ## if both parents are called and both do not have the same genotype
...
def Null_mapper(sex_linked_markers_path, catalog_path, Genome_fasta, Genome_db, N_its = 10, Species = "", Threads = 1):
    ...
    max_y = max([max(i) for i in list(All_mapping_counts.values())] + list(Real_SL_tagXen_chroms.values())) + 3
...
def Summary_plotter(input_vcf_path, switches = 111, sort = False):
    ...
    if switches[0] == "1": ## make the missing data plot
        ...
        if sort == True:
            ...
            missing_sorted_sample_names = [i[0] for i in sorted(list(per_sample_missing_data.items()), key=operator.itemgetter(1), reverse=True)]
        ...
        ax1.hlines(np.mean(list(per_sample_missing_data.values())), 0, len(per_sample_missing_data))
    ...
    if switches[1] == "1": ## make the heterozygosity plot
        ...
        if sort == True:
            ...
            heterozygosity_sorted_sample_names = [i[0] for i in sorted(list(prop_hets.items()), key=operator.itemgetter(1), reverse=True)]  
        else:
        ...
        ax2.hlines(np.mean(list(prop_hets.values())), 0, len(per_sample_missing_data))
    ...
    if switches[2] == "1": ## make the coverage plot
        ...
        overall_mean = np.mean(list(per_sample_coverage_means.values()))
        ...
        if sort == True:
            ...
            coverage_sorted_sample_names = [i[0] for i in sorted(list(per_sample_coverage_means.items()), key=operator.itemgetter(1), reverse=True)]  
        else:
