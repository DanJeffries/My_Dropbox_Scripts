...
def Perm_plotter(Rand_dir, Parameter_dictionaries, multiplier = 10):

    """
    Rand_dir               -  Directory containing randomisations and real sex assignment results
    Parameter_dictionaries -  The parameter dictionaries used for the permutations

    ** Real sex assignmnet results must be in a file named "Real_sex_assignment_SLMs.txt"
    ** Permuation results must be in files whose names start with "Randomisations"
    TESTING    
    """
    ...
    ## get the real sex assignment data

    real_data = open("%s/Real_sex_assignment_SLMs.txt" % Rand_dir, 'r').readlines()
    ...
    for i in real_data[1:]:
        name = i.split()[0]
        real_data_dict[name] = {}
        real_data_dict[name]["XY_freqs"] = int(i.split()[1])
        real_data_dict[name]["XY_hets"] = int(i.split()[2])
        real_data_dict[name]["XY_specs"] = int(i.split()[3])
        real_data_dict[name]["ZW_freqs"] = int(i.split()[4])
        real_data_dict[name]["ZW_hets"] = int(i.split()[5])
        real_data_dict[name]["ZW_specs"] = int(i.split()[6])



    ## Now get the permutation data


    permutation_dict = {}

    for permutation_output in os.listdir(Rand_dir):
        if permutation_output.startswith("Randomisations"):
            perm_file = open("%s/%s" % (Rand_dir, permutation_output), 'r')
        ...
        for perm in perm_file:
            if perm.split()[0] not in permutation_dict:
                permutation_dict[perm.split()[0]] = {}

            XY_freqs.append(int(perm.split()[1]))
            XY_hets.append(int(perm.split()[2]))
            XY_specs.append(int(perm.split()[3]))
            ZW_freqs.append(int(perm.split()[4]))
            ZW_het.append(int(perm.split()[5]))
            ZW_spec.append(int(perm.split()[6]))

            permutation_dict[perm.split()[0]]["XY_freqs"] = (np.round(np.mean(XY_freqs)),np.round(np.percentile(XY_freqs,95)))
            permutation_dict[perm.split()[0]]["XY_hets"] = (np.round(np.mean(XY_hets)),np.round(np.percentile(XY_hets,95)))
            permutation_dict[perm.split()[0]]["XY_specs"] = (np.round(np.mean(XY_specs)),np.round(np.percentile(XY_specs,95)))
            permutation_dict[perm.split()[0]]["ZW_freqs"] = (np.round(np.mean(ZW_freqs)),np.round(np.percentile(ZW_freqs,95)))
            permutation_dict[perm.split()[0]]["ZW_hets"] = (np.round(np.mean(ZW_het)),np.round(np.percentile(ZW_het,95)))
            permutation_dict[perm.split()[0]]["ZW_spec"] = (np.round(np.mean(ZW_spec)),np.round(np.percentile(ZW_spec,95)))
    ...
    for method in ["freqs", "hets"]:#, "specs"]:
        ...
        for param_set in real_data_dict:
            XY_perm_mean = permutation_dict[param_set]['XY_%s' % method][0]
            XY_perm_95th = permutation_dict[param_set]['XY_%s' % method][1]

            ZW_perm_mean = permutation_dict[param_set]['ZW_%s' % method][0]
            ZW_perm_95th = permutation_dict[param_set]['ZW_%s' % method][1]

            XY_real = real_data_dict[param_set]['XY_%s' % method]
            ZW_real = real_data_dict[param_set]['ZW_%s' % method]

            for param_dict in Parameter_dictionaries:
                param_dict = Parameter_dictionaries[param_dict]
                if param_dict["Name"] == param_set:

                    if method == "freqs":
                        x = param_dict['homogametic_REF_allele_freq']
                        xs.append(x)
                        y = param_dict['X_or_Z_freq_threshold']
                        ys.append(y)
                    elif method == "hets":
                        x = param_dict['homogamtic_homozygosity_threshold']
                        xs.append(x)
                        y = param_dict['heterogamtic_heterozygosity_threshold']
                        ys.append(y)



            plt.scatter(x-0.01, y, s=XY_real*multiplier, c = "blue")#, zorder = XY_real_zorder)
            plt.scatter(x+0.01, y, s=ZW_real*multiplier, c = "red")#, zorder = ZW_real_zorder)

            plt.scatter(x-0.01, y, s=(XY_perm_95th)*multiplier, c = "grey")#, zorder = XY_perm_zorder)
            plt.scatter(x+0.01, y, s=(ZW_perm_95th)*multiplier, c = "grey")#, zorder = ZW_perm_zorder)



            if all([XY_real>0, XY_perm_95th > 0]):
                plt.text(x-0.01, y+0.0075, np.round(XY_real / XY_perm_95th, 2), horizontalalignment='center')
            if all([XY_real>0, XY_perm_95th == 0]):
                plt.text(x-0.01, y+0.0075, np.round(XY_real / 1, 2), horizontalalignment='center')

            if all([ZW_real>0, ZW_perm_95th > 0]):
                plt.text(x+0.01, y+0.0075, np.round(ZW_real / ZW_perm_95th, 2), horizontalalignment='center')
            if all([ZW_real>0, ZW_perm_95th == 0]):
                plt.text(x+0.01, y+0.0075, np.round(ZW_real / 1, 2), horizontalalignment='center')


        plt.xlabel("Homogametic threshold")
        plt.ylabel("Heterogametic threshold")
    ...
    for param_set in sorted(real_data_dict.keys()):
        ZW_randoms.append(permutation_dict[param_set]['ZW_spec'])
        XY_randoms.append(permutation_dict[param_set]['XY_specs'])
        ZW_real.append(real_data_dict[param_set]['ZW_specs'])
        XY_real.append(real_data_dict[param_set]['XY_specs'])


    plt.subplot(1,3,subplot_index)

    x1= range(1,(len(ZW_randoms))*6, 6)
    x2 = [i+1 for i in x1]
    xticksmarks = [i+2 for i in x1]
    x3= [i+3 for i in x1]
    x4= [i+4 for i in x1]

    XYys = [i[1] for i in XY_randoms]
    ZWys = [i[1] for i in ZW_randoms]

    plt.bar(x1,XYys, color = "darkgrey") ## perms XY
    plt.bar(x2,XY_real, color = "blue") ## real XY

    plt.bar(x3,ZWys, color = "darkgrey") ## perms ZW
    plt.bar(x4,ZW_real, color = "red") ## real ZW


    plt.xticks(xticksmarks, sorted(permutation_dict.keys()), rotation = 90)

    all_Ys = XYys+ZWys+XY_real+ZW_real

    plt.yticks(range(0,int(max(all_Ys))+1))
    plt.ylabel("")
    
    plt.savefig("%s/Randomisation_plots.pdf" % Rand_dir)
    print "Figures saved here: %s/Randomisation_plots.pdf" % Rand_dir
...
def SLM_permutations(Parameter_dictionaries, Dict_template = False, N_perms = 100, threads = 6, template = False, plot = True, real_only = False):

    """
    Will run permutations for finding sex linked markers using SLM_finder, each time shuffling the 
    assignments of males and females over the dataset. This gives a null distribution of the expected
    number of false positive XY and ZW linked markers for each of the three SLM_finder methods. 
    
    Parameter_dictionaries - A LIST of dictionaries containing the SLM_finder arguments for the
                             permutations. Run template = True to print dictionary template.
                             
    N_perms                - The number of permutations to perform. Default is 100. 
    
    threads                - Number of jobs to run simultaneously. Default is 6.
    
    template               - Output a dictionary with the appropriate structure to be filled with the
                             necassary information. Must be placed within a list of dictionaries to be passed 
                             as Parameter_dictionaries.
                             
    plot                   - True or False, whether to plot. 
    
    real_only              - Only run the real sex linked marker tests (not permutations)
    
    """
    
    

    from joblib import Parallel, delayed  ## joblib 0.12.1 works, haven't tried with later versions but 0.11 fails. 
    ...
    ## Define a small function to help parallelise the analyses
    ...
    ## Output dictionary template if requested
    
    if template == True:
        
        Parameter_dict_template = {}
        Parameter_dict_template["Name"] = "Param_set_name"

        ##### Data ########################

        Parameter_dict_template['Catalog'] =  "full/path/to/catalog.tags.tsv.gz" ## Path to the catalog file - used by all approaches.
        Parameter_dict_template['VCF'] =  "full/path/to/batch_1.vcf" ## path to vcf file (note this will be altered to make header compatible with Pyvcf. New vcf will have same name with ".altered" appended to the end). Used by Approach i) and ii)
        Parameter_dict_template['Pop_map'] = "full/path/to/Sex_ID_info.txt" ## path to population map file containing sex information. Same format as Stacks pop map file. Used by all approaches.

        ###### threshold parameters #######

        # 1. Frequency approach
        Parameter_dict_template['X_or_Z_freq_threshold'] = 0.4  ## (Default = 0.4) The lower threshold for the freq caluclation to find sex linked snps, e.g. for an XY system, a threshold of 0.4 means that f(F) - f(M) can be >= 0.4 and <= 0.6 (the upper threshold is automatically calculated to be the same distance above 0.5 as the lower threshold is below 0.5) 
        Parameter_dict_template['sample_presence_cutoff1'] = 0.75 ## (Default = 0.75) a locus must be called in at least this proportion of all samples (not within populations) to be considered
        Parameter_dict_template['coverage_threshold1'] = 3 ## (Default = 3) a locus must have at least this threshold in a sample to be considered for that sample. Note that loci below this threshold will be removed from a sample, and this can push the locus below the sample presence cut-off, which will then remove the locus.
        Parameter_dict_template['maf_threshold1'] =  0.05 ## (Default = 0.05) minor allele frequency cutoff for a locus across all samples. 
        Parameter_dict_template['homogametic_REF_allele_freq'] = 0.95 ## (Default = 0.95) The sex linked SNP will be the minor allele, so a check is done to make sure that the homogametic sex is above the threshold specified for the major allele. In theory this should be 1. But allowing for some error 0.95 is used as a default. 

        # 2. Heterozygosity approach
        Parameter_dict_template['homogamtic_homozygosity_threshold'] = 0.9 ## (Default = 0.9) The minimum number of the homogametic sex which must not have the tag for that tag to be considered linked to the sex-limited chromosome
        Parameter_dict_template['heterogamtic_heterozygosity_threshold'] = 0.5 ## (Default = 0.5) The lower threshold for the proportion of heterozygotes in the heterogametic sex at a locus 
        Parameter_dict_template['sample_presence_cutoff2'] = 0.75 ## (Default = 0.75) a locus must be called in at least this proportion of all samples (not within populations) to be considered
        Parameter_dict_template['coverage_threshold2'] = 3 ## (Default = 3) a locus must have at least this threshold in a sample to be considered for that sample. Note that loci bels this threshold will be removed from a sample, and this can push the locus below the sample presence cut-off, which will then remove the locus.
        Parameter_dict_template['maf_threshold2'] = 0.05 ## (Default = 0.05) minor allele frequency cutoff for a locus across all samples. 

        # 3. Sex specific presence or absence approach
        Parameter_dict_template['sex_presence_threshold'] =  0.5 ## (Default = 0.5) The minimum percenatage of the heterogametic sex that a tag must be present in.

        return Parameter_dict_template
        ...
        for dataset in Parameter_dictionaries:
            dataset = Parameter_dictionaries[dataset]
            
            if real_only == False:
                print("processing dataset in", dataset["VCF"])

            # 1. Make a new directory in the VCF path. 
            ...
            if real_only == False:
                ...
                results_dict[dataset["Name"]] = Parallel(n_jobs=threads, verbose = 1)(delayed(Super_SLM_finder_parallel)(i, dataset) for i in popmaps)
        ...
        ## Running real SLM finder
    
        print "\nRunning SLM_finder for real sex assignments:"
        
        real_results_handle = open("%s/Real_sex_assignment_SLMs.txt" % Randomisation_dir, 'w')
        real_results_handle.write("Param_set\tXY_freq\tXY_het\tXY_spec\tZW_freq\tZW_het\tZW_spec\n")
        ...
        for Param_dict in Parameter_dictionaries:
            ...
            print "\t%s" % Param_dict["Name"]

            Xlinked_freq, Zlinked_freq, freq_log = SLMF_L.SL_snp_freq(Param_dict['VCF'], 
                                                                      Param_dict['Pop_map'],
                                                                      Param_dict['Catalog'],
                                                                      Param_dict['X_or_Z_freq_threshold'],
                                                                      Param_dict['sample_presence_cutoff1'],
                                                                      Param_dict['coverage_threshold1'],
                                                                      Param_dict['maf_threshold1'],
                                                                      Param_dict['homogametic_REF_allele_freq'],
                                                                      verbose = False,
                                                                      plot = False)
            
            Xlinked_het, Zlinked_het, het_log = SLMF_L.SL_snp_het(Param_dict['VCF'],
                                                                  Param_dict['Pop_map'],
                                                                  Param_dict['Catalog'],
                                                                  Param_dict['homogamtic_homozygosity_threshold'],
                                                                  Param_dict['heterogamtic_heterozygosity_threshold'],
                                                                  Param_dict['sample_presence_cutoff2'],
                                                                  Param_dict['coverage_threshold2'],
                                                                  Param_dict['maf_threshold2'],
                                                                  verbose = False)
            
            Ylinked_tags, Wlinked_tags, pres_abs_log = SLMF_L.SL_tag_finder(Param_dict['Catalog'],
                                                                            Param_dict['Pop_map'],
                                                                            Param_dict['sex_presence_threshold'],
                                                                            verbose = False)
            
            
            
            real_results_dict[Param_dict["Name"]] = {}
            real_results_dict[Param_dict["Name"]]["XY_freq"] = len(Xlinked_freq)
            real_results_dict[Param_dict["Name"]]["XY_het"] = len(Xlinked_het)
            real_results_dict[Param_dict["Name"]]["XY_spec"] = len(Ylinked_tags)
            real_results_dict[Param_dict["Name"]]["ZW_freq"] = len(Zlinked_freq)
            real_results_dict[Param_dict["Name"]]["ZW_het"] = len(Zlinked_het)
            real_results_dict[Param_dict["Name"]]["ZW_spec"] = len(Wlinked_tags)
            
            real_results_handle.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (Param_dict["Name"], len(Xlinked_freq), len(Xlinked_het), 
                                                                        len(Ylinked_tags), len(Zlinked_freq), len(Zlinked_het), len(Wlinked_tags)))
            
        print "\nReal sex assignment results outputted to here: %s/Real_sex_assignment_SLMs.txt" % Randomisation_dir
...
def fasta_maka(whitey, cat, out = None):
    ...
    print "Number of markers in whitelist:",len(Loc_IDs)
...
def BlastParseExtra(infile, genome_fa, best_hit_criteria, Eval_thresh, get_frags_switch = 1, window_size = 2000, verb = 0):
    ...
    if verb == 1:
        ...
        print "Number of tags which aligned at least once:", alignment_counter
        print "Number of multi-alignments kept:", multi_counter
        print "Number of unique alignments kept:", unique_counter
	print "Number of multi-alignments removed:", multi_removed
	print "Number of unique alignments removed:", unique_removed
...
def Summary_plotter(input_vcf_path, switches = 111, sort = False):
    ...
    switches = a STRING of four 0 or 1 switches for the missing data, coverage and heterozygosity plots in that order. Example: "1101". - default "1111"
    sort = True or False - whether to sort the samples by the plotted metric (True), or keep order in vcf (False) - default False
    """
	...
	Av_covs = [] ## record averages to get overall average for the y axis.

        for sample in coverage_sorted_sample_names:
            
            x_tick_pos.append(vio_pos)
            x_tick_labs.append(sample)
            Av_covs.append(np.mean(per_sample_coverage[sample]))
            ...
            vio_pos += 1
	ax3.set_ylim(0, np.percentile(Av_covs, 99))   
        plt.xticks(x_tick_pos, x_tick_labs, rotation = 90)
